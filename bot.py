    ...
    <main>
        <div class="admin-container">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="action-buttons">
                                    ...
                                    <a href="{{ url_for('admin_delete', content_id=content._id) }}" class="delete-btn" onclick="return confirm('Are you sure you want to delete this content?');">Delete</a>
                                </td>
                            </tr>
                            {% endfor %}
                        </tbody>
            ...
            {% elif page_type == 'edit' %}
                ...
                <form method="post" action="{{ url_for('admin_edit', content_id=content._id) }}" class="admin-form">
    ...
    try:
        ...
        if res.get('results'):
            ...
            if parsed_info['type'] == 'movie': 
                title, year = data.get('title'), (data.get('release_date', '') or '')[:4]
            else: 
...
def content_detail(content_id):
    ...
    try:
        ...
        if content:
            return render_template_string(DETAIL_TEMPLATE, content=content, css_code=CSS_CODE)
        else:
            abort(404)
    except Exception as e:
...
def process_new_content(file_data, message_id):
    ...
    if not (tmdb_data and tmdb_data.get('original_title')):
        print(f"Could not get TMDb info for: {parsed_info.get('title')}")
        return
