...
from pymongo import MongoClient, DESCENDING  # <--- পরিবর্তন এখানে
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logo { font-size: 2rem; font-weight: 700; color: var(--other-color); text-decoration: none; }
        ...
        .nav-link.active { color: var(--other-color); }
        ...
        .footer { padding: 40px 20px; border-top: 1px solid #222; margin-top: 40px; }
    </style>
...
<body>
    ...
    <footer class="footer">
        <div class="container text-center" style="color: var(--text-grey);">
            <p>This site does not store any file on the server, it only links to media files which are hosted on Telegram.</p>
            <p class="mt-4">© {{ now.year }} {{ sitename }}. All Rights Reserved.</p>
        </div>
    </footer>
</body>
</html>
...
{% extends "base_layout.html" %}
{% block content %}
<div class="detail-hero" style="position: relative; min-height: 80vh; display: flex; align-items: center; padding: 40px;">
    ...
    <div style="position: relative; display: flex; gap: 40px; align-items: center;">
        <div class="detail-poster">
            <img src="{{ content.poster or 'https://via.placeholder.com/400x600.png?text=No+Image' }}" alt="{{ content.title }}" style="width: 300px; border-radius: 8px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
        </div>
        <div class="detail-info">
            <h1 style="font-size: 3rem; margin-bottom: 20px;">{{ content.title }}</h1>
            <div class="flex items-center flex-wrap gap-4 mb-4" style="color: var(--text-grey);">
                ...
                {% if content.type == 'movie' and content.files %}
                    {% for file in content.files|sort(attribute='quality', reverse=True) %}
                        <div class="flex items-center gap-4">
                            <a href="{{ url_for('player', movie_id=content._id, quality=file.quality) }}" target="_blank" class="flex items-center gap-2" style="background-color: var(--other-color); color: black; padding: 10px 20px; border-radius: 5px;">
                                <i class="fas fa-play"></i> Play {{ file.quality }}
                            </a>
                            <a href="{{ url_for('download_file', movie_id=content._id, quality=file.quality) }}" class="flex items-center gap-2" style="background-color: #333; color: white; padding: 10px 20px; border-radius: 5px;">
                                <i class="fas fa-download"></i> Download {{ file.quality }}
                            </a>
                        </div>
                    {% endfor %}
...
def home():
    ...
    return render_template_string(home_html, base_layout_html=base_layout_html, hero_movie=hero_movie, latest_movies=process_movie_list(latest_movies), seo=seo)
...
def movie_detail(tmdb_id):
    ...
    return render_template_string(detail_html, base_layout_html=base_layout_html, content=process_movie_list([content])[0], seo=seo)
...
def telegram_webhook():
    ...
    if not parsed_info or not parsed_info.get('title'):
        print(f"Webhook FATAL: Could not parse title from '{filename}'"); return jsonify(status='ok')
    ...
    if not tmdb_data:
        print(f"Webhook FATAL: Could not find TMDb data for '{parsed_info['title']}'"); return jsonify(status='ok')
...
@requires_auth
def admin():
    return render_template_string(admin_html, base_layout_html=base_layout_html)
